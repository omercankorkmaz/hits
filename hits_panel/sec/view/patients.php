...
<?php if($_SESSION['time'] < time()){header('Location:' . site_url()); session_destroy();} else{$_SESSION['time'] = time() + 900;}?>
    ...
    $(window).scroll(function() {
      var scrollHeight = $(document).height();
    	var scrollPosition = $(window).height() + $(window).scrollTop();
         ...
         limit = limit + 10;
         $.post('controller/ajax_patient.php',{'limit':limit},function(response){
  ...
  $(function(){
    ...
    $.post('controller/ajax_patient.php',function(response){
      $('.loader').hide();
      $('#table').html(response);
  });
  ...
  $('#name').on('change', function(e){
      var data = $('#name').val();
      $.post('controller/ajax_patient.php',{'name':data, 'limit':limit},function(response){
        $('#table').html(response);
    });
  });
  ...
  $('#reset').on('click', function(e){
    $('#name').val('');
    $.post('controller/ajax_patient.php',{'name':'', 'limit':limit},function(response){
        $('#table').html(response);
  });
  });
});
...
$(function(){
  ...
  $('#table').on("click",function(e){
    if((e.target.localName) == 'a'){
        $.post('controller/ajax_patient_add_box.php', {'add' : c} , function(response){
          $('.modal-body-bottom').append(response);
        }).done(function(){
          $.post('controller/ajax_patient_add_box.php', {'select' : c} , function(response){
              $(`.slt`).html(response);
          }).done(function(){
            c = c +1;
          });
    ...
    else if((e.target.id).includes('cancel') || (event.target.className) == 'modal fade'){
  ...
  if(e.target.className.includes('crt')){
        $.post('controller/ajax_patient.php', {'data':e.target.className} ,function(response){
